const OWNER_ID="293469802188636160";
...
export async function onRequestGet({env,request}) {
  ...
  const me=await fetch("https://discord.com/api/v10/users/@me",{headers:{Authorization:`Bearer ${t.access_token}`}});
  if(!me.ok)return Response.redirect(u.origin+"/?auth=failed",302);
  const user=await me.json();
  if(user.id!==OWNER_ID)return Response.redirect(u.origin+"/?auth=denied",302);
  const session=await sign({id:user.id,username:user.username,global_name:user.global_name||user.username,avatar:user.avatar,exp:Date.now()+86400000},env.SESSION_SECRET);
