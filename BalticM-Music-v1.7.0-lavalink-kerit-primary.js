...
if (!LAVALINK_HOST || !LAVALINK_PORT || !LAVALINK_PASSWORD) {
  console.warn(
    "[BalticM Lavalink] Kerit env vars incomplete; using public fallback nodes only."
  );
}
...
shoukaku.on("ready", name => console.log(`[BalticM Lavalink] READY: ${name}`));
...
async function connectPlayer(guildId, channelId) {
  ...
  if (!state) {
    state={guildId,channelId,channelName:channel.name,queue:[],volume:100,currentTrack:null,paused:false,playbackError:null,connectedAt:Date.now()};
  ...
  if (!state.lavalinkEventsBound) {
    ...
    lava.on("exception", data => {
      const error = new Error(`Lavalink track exception: ${data?.exception?.message || "unknown"}`);
      const p = players.get(guildId);
      if (p) p.playbackError = error;
      setLastError(error);
    });
    lava.on("stuck", data => {
      const error = new Error(`Lavalink track stuck: ${data?.thresholdMs || "unknown"}ms`);
      const p = players.get(guildId);
      if (p) p.playbackError = error;
      setLastError(error);
    });
...
async function playNext(guildId) {
  ...
  if (!next) {
    console.log(`[BalticM Music] ${guildId}: queue empty`);
    return null;
  }
  ...
  try {
    ...
    const metadata = await resolveLavalinkTrack(next);
    ...
    await lava.setGlobalVolume(
      Math.max(0, Math.min(200, Number(player.volume || 100)))
    );

    // Lavalink can accept playTrack and then emit an asynchronous track
    // exception moments later. Give it a short verification window so the
    // HTTP /play response does not falsely claim "Now playing".
    await new Promise(resolve => setTimeout(resolve, 1800));

    if (player.playbackError) {
      const error = player.playbackError;
      player.playbackError = null;
      player.currentTrack = null;
      throw error;
    }

    voiceLog("PLAYBACK VERIFIED", {
      guildId,
      title: next.title,
    ...
    console.log(
      `[BalticM Music] Now playing via Lavalink: ${next.title} [${lava.node?.name || "node"}]`
    );
