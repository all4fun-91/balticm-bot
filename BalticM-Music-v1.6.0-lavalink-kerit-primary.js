...
async function connectPlayer(guildId, channelId) {
  ...
  if (!state.lavalinkEventsBound) {
    ...
    lava.on("exception", data => {
      setLastError(new Error(`Lavalink track exception: ${data?.exception?.message || "unknown"}`));
    });
    lava.on("stuck", data => {
      setLastError(new Error(`Lavalink track stuck: ${data?.thresholdMs || "unknown"}ms`));
    });
  }
...
async function resolveLavalinkTrack(track) {
  ...
  const identifier = track?.playbackUrl || track?.originalUrl || `ytsearch:${track?.artist ? track.artist + " " : ""}${track?.title || ""}`;
  const result = await node.rest.resolve(identifier);
  const list = result?.data && Array.isArray(result.data) ? result.data : (result?.data ? [result.data] : []);
  const found = list[0] || result?.tracks?.[0] || null;
  if (!found?.encoded) throw new Error(`Lavalink could not resolve: ${track?.title || identifier}`);
  return found;
}
        ...
        if (
          ...
          return json(
            ...
            200,
            {
              ...
              version:
                "1.5.1-lavalink-millohost-primary",
