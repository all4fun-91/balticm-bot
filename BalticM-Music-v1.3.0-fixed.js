...
async function connectPlayer(
  ...
  if (
    ...
    console.log(
      "[BalticM Voice] REUSING READY CONNECTION"
    );
  ...
  if (connection) {
    console.log(
      "[BalticM Voice] DESTROY OLD CONNECTION",
  ...
  console.log(
    "[BalticM Voice] CALL joinVoiceChannel"
  );
  ...
  connection.on(
    ...
    (oldState, newState) => {
      console.log(
        `[BalticM Voice] STATE ${oldState.status} -> ${newState.status}`
      );
    }
  );
  ...
  connection.on(
    ...
    error => {
      console.error(
        "[BalticM Voice] CONNECTION ERROR",
        error
      );
    }
  );
  ...
  console.log(
    "[BalticM Voice] AUDIO PLAYER SUBSCRIBED"
  );

  try {
    console.log(
      "[BalticM Voice] WAIT READY 20000ms"
    );
  ...
  console.log(
    `[BalticM Music] Connected to ${channel.name} (${channel.id})`
  );
