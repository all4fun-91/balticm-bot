...
async function giveawayState(env,guildId){await ensureGiveawayTables(env);const r=await env.BALTICM_DB.prepare("SELECT g.id,g.channel_id AS channelId,g.prize,g.description,g.winner_count AS winnerCount,g.end_at AS endAt,g.required_roles_json AS requiredRolesJson,g.excluded_roles_json AS excludedRolesJson,g.message_id AS messageId,g.status,g.winner_ids_json AS winnerIdsJson,g.created_at AS createdAt,g.updated_at AS updatedAt,g.ended_at AS endedAt,(SELECT COUNT(*) FROM giveaway_entries e WHERE e.giveaway_id=g.id) AS entryCount FROM giveaways g WHERE g.guild_id=? ORDER BY g.created_at DESC").bind(guildId).all();return json({giveaways:(r.results||[]).map(giveawayRow)})}
