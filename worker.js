...
async function ensureGiveawayTables(env){
 ...
 try{await env.BALTICM_DB.prepare("ALTER TABLE giveaways ADD COLUMN required_roles_mode TEXT NOT NULL DEFAULT 'all'").run()}catch{}
...
async function giveawayEnterInteraction(env,interaction,id){await ensureGiveawayTables(env);const guildId=String(interaction.guild_id||""),userId=String(interaction.member?.user?.id||interaction.user?.id||"");const row=await env.BALTICM_DB.prepare("SELECT id,channel_id AS channelId,prize,description,winner_count AS winnerCount,end_at AS endAt,required_roles_json AS requiredRolesJson,excluded_roles_json AS excludedRolesJson,required_roles_mode AS requiredRolesMode,message_id AS messageId,status FROM giveaways WHERE id=? AND guild_id=?").bind(id,guildId).first();if(!row)return json({type:4,data:{content:"This giveaway no longer exists.",flags:64}});const g=giveawayRow(row);if(g.status!=="active"||Date.parse(g.endAt)<=Date.now())return json({type:4,data:{content:"This giveaway has ended.",flags:64}});if(interaction.member?.user?.bot)return json({type:4,data:{content:"Bots cannot enter giveaways.",flags:64}});const bad=await giveawayEligible(env,g,userId,interaction.member?.roles||[]);if(bad)return json({type:4,data:{content:bad,flags:64}});const existing=await env.BALTICM_DB.prepare("SELECT user_id FROM giveaway_entries WHERE giveaway_id=? AND user_id=?").bind(id,userId).first();if(existing){await env.BALTICM_DB.prepare("DELETE FROM giveaway_entries WHERE giveaway_id=? AND user_id=?").bind(id,userId).run();const payload=await giveawayPayload(env,g);if(g.messageId)await fetch("https://discord.com/api/v10/channels/"+g.channelId+"/messages/"+g.messageId,{method:"PATCH",headers:botHeaders(env,true),body:JSON.stringify(payload)}).catch(()=>null);return json({type:4,data:{content:"🎉 You left the giveaway.",flags:64}})}await env.BALTICM_DB.prepare("INSERT INTO giveaway_entries (giveaway_id,guild_id,user_id,entered_at) VALUES (?,?,?,?)").bind(id,guildId,userId,new Date().toISOString()).run();const payload=await giveawayPayload(env,g);if(g.messageId)await fetch("https://discord.com/api/v10/channels/"+g.channelId+"/messages/"+g.messageId,{method:"PATCH",headers:botHeaders(env,true),body:JSON.stringify(payload)}).catch(()=>null);return json({type:4,data:{content:"🎉 You're entered! Good luck.",flags:64}})}
...
if(interaction?.type===3&&customId.startsWith("giveaway_enter:")){try{return await giveawayEnterInteraction(env,interaction,customId.slice("giveaway_enter:".length))}catch(e){return json({type:4,data:{content:"Could not update your giveaway entry.",flags:64}})}}
