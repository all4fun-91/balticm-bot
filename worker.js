...
async function supportChatState(req,env,user){
 try{
  ...
  if(markRead&&Number(thread.unreadUser)>0)await env.BALTICM_DB.prepare("UPDATE bot_support_threads SET unread_user=0 WHERE id=? AND user_id=?").bind(thread.id,String(user.id)).run();
  return json({thread:{...thread,unreadUser:markRead?0:Number(thread.unreadUser||0)},messages:rows.results||[]});
...
async function reactionRoleState(env,guildId){
 ...
 const [p,l]=await Promise.all([
  env.BALTICM_DB.prepare("SELECT id,channel_id AS channelId,title,description,thumbnail_url AS thumbnailUrl,message_id AS messageId,enabled,created_at AS createdAt,updated_at AS updatedAt FROM reaction_role_panels WHERE guild_id=? ORDER BY created_at DESC").bind(guildId).all(),
...
async function saveReactionRolePanel(req,env,guildId){
 let body;try{body=await req.json()}catch{return json({error:"Invalid JSON"},400)}
 const channelId=String(body.channelId||"").trim(),title=String(body.title||"").trim().slice(0,256),description=String(body.description||"").trim().slice(0,2000),thumbnailUrl=String(body.thumbnailUrl||"").trim().slice(0,1000);
...
async function updateReactionRolePanel(req,env,guildId,id){
 ...
 const channelId=String(body.channelId||"").trim(),title=String(body.title||"").trim().slice(0,256),description=String(body.description||"").trim().slice(0,2000),thumbnailUrl=String(body.thumbnailUrl||"").trim().slice(0,1000);
 const links=(Array.isArray(body.links)?body.links:[]).map(x=>({emoji:String(x.emoji||"").trim().slice(0,100),roleId:String(x.roleId||"").trim(),label:String(x.label||"").trim().slice(0,100)})).filter(x=>x.emoji&&/^\d{16,22}$/.test(x.roleId));
 if(!/^\d{16,22}$/.test(channelId)||!title||!links.length)return json({error:"Channel, title and at least one mapping are required"},400);if(thumbnailUrl&&!/^https:\/\//i.test(thumbnailUrl))return json({error:"Thumbnail URL must use https://"},400);
 const [cr,rr]=await Promise.all([fetch(`https://discord.com/api/v10/channels/${channelId}`,{headers:botHeaders(env)}),fetch(`https://discord.com/api/v10/guilds/${guildId}/roles`,{headers:botHeaders(env)})]);if(!cr.ok)return json({error:"Could not access selected channel"},400);const ch=await cr.json();if(String(ch.guild_id||"")!==String(guildId)||ch.type!==0)return json({error:"Select a text channel from this server"},400);const roles=rr.ok?await rr.json():[],roleIds=new Set(roles.map(r=>r.id));if(links.some(x=>!roleIds.has(x.roleId)))return json({error:"One or more selected roles are invalid"},400);
 await env.BALTICM_DB.prepare("UPDATE reaction_role_panels SET channel_id=?,title=?,description=?,thumbnail_url=?,updated_at=? WHERE id=? AND guild_id=?").bind(channelId,title,description,thumbnailUrl,new Date().toISOString(),id,guildId).run();await env.BALTICM_DB.prepare("DELETE FROM reaction_role_links WHERE panel_id=? AND guild_id=?").bind(id,guildId).run();for(const link of links)await env.BALTICM_DB.prepare("INSERT INTO reaction_role_links (id,panel_id,guild_id,emoji,role_id,label) VALUES (?,?,?,?,?,?)").bind(crypto.randomUUID(),id,guildId,link.emoji,link.roleId,link.label).run();return json({ok:true});
}
...
async function ensureGiveawayTables(env){
 ...
 try{await env.BALTICM_DB.prepare("ALTER TABLE giveaways ADD COLUMN required_roles_mode TEXT NOT NULL DEFAULT 'all'").run()}catch{} try{await env.BALTICM_DB.prepare("ALTER TABLE giveaways ADD COLUMN logo_url TEXT NOT NULL DEFAULT ''").run()}catch{}
...
function giveawayRow(x){let requiredRoles=[],excludedRoles=[],winnerIds=[];try{requiredRoles=JSON.parse(x.requiredRolesJson||"[]")}catch{}try{excludedRoles=JSON.parse(x.excludedRolesJson||"[]")}catch{}try{winnerIds=JSON.parse(x.winnerIdsJson||"[]")}catch{}return {...x,requiredRoles,excludedRoles,winnerIds,entryCount:Number(x.entryCount||0)}}
async function giveawayState(env,guildId){await ensureGiveawayTables(env);const r=await env.BALTICM_DB.prepare("SELECT g.id,g.channel_id AS channelId,g.prize,g.description,g.logo_url AS logoUrl,g.winner_count AS winnerCount,g.end_at AS endAt,g.required_roles_json AS requiredRolesJson,g.excluded_roles_json AS excludedRolesJson,required_roles_mode AS requiredRolesMode,g.message_id AS messageId,g.status,g.winner_ids_json AS winnerIdsJson,g.created_at AS createdAt,g.updated_at AS updatedAt,g.ended_at AS endedAt,(SELECT COUNT(*) FROM giveaway_entries e WHERE e.giveaway_id=g.id) AS entryCount FROM giveaways g WHERE g.guild_id=? ORDER BY g.created_at DESC").bind(guildId).all();return json({giveaways:(r.results||[]).map(giveawayRow)})}
...
async function giveawayPayload(env,g){const branding=await giveawayBrandingFooter(env,g.guildId);const er=await env.BALTICM_DB.prepare("SELECT COUNT(*) AS c FROM giveaway_entries WHERE giveaway_id=?").bind(g.id).first(),count=Number(er?.c||0),endUnix=Math.floor(Date.parse(g.endAt)/1000);return{embeds:[{title:"🎉 GIVEAWAY — "+g.prize,description:[g.description||"Enter for a chance to win!",`
...
**Entries:** ${count}`].filter(Boolean).join("\n"),color:0x7457ff,...(g.logoUrl?{thumbnail:{url:g.logoUrl}}:{}),...branding}],components:g.status==="ended"?[]:[{type:1,components:[{type:2,style:3,label:"Enter Giveaway",emoji:{name:"🎉"},custom_id:"giveaway_enter:"+g.id}]}],allowed_mentions:{parse:[]}}}
...
function announcementRow(x){if(!x)return null;let buttons=[];try{buttons=JSON.parse(x.buttonsJson||"[]")}catch{}return Object.assign({},x,{crossposted:!!x.crossposted,buttons:buttons})}
async function announcementState(env,guildId){await ensureAnnouncementTables(env);const r=await env.BALTICM_DB.prepare("SELECT id,channel_id AS channelId,title,content,thumbnail_url AS thumbnailUrl,image_url AS imageUrl,color,footer,buttons_json AS buttonsJson,message_id AS messageId,crossposted,status,created_at AS createdAt,updated_at AS updatedAt,published_at AS publishedAt FROM announcements WHERE guild_id=? ORDER BY created_at DESC").bind(guildId).all();return json({announcements:(r.results||[]).map(announcementRow)})}
function parseAnnouncementBody(body){const channelId=String(body.channelId||"").trim(),title=String(body.title||"").trim().slice(0,256),content=String(body.content||"").trim().slice(0,4000),thumbnailUrl=String(body.thumbnailUrl||"").trim().slice(0,1000),imageUrl=String(body.imageUrl||"").trim().slice(0,1000),footer=String(body.footer||"").trim().slice(0,2048),color=Math.max(0,Math.min(16777215,Number(body.color)||7624695)),buttons=(Array.isArray(body.buttons)?body.buttons:[]).slice(0,5).map(x=>({label:String(x.label||"").trim().slice(0,80),url:String(x.url||"").trim().slice(0,1000),emoji:String(x.emoji||"").trim().slice(0,100)})).filter(x=>x.label&&/^https:\/\//i.test(x.url));return{channelId,title,content,thumbnailUrl,imageUrl,footer,color,buttons,crosspost:!!body.crosspost}}
async function validateAnnouncement(env,guildId,a){if(!/^\d{16,22}$/.test(a.channelId))return"Select a Discord channel";if(!a.title&&!a.content)return"Add a title or message";if(a.thumbnailUrl&&!/^https:\/\//i.test(a.thumbnailUrl))return"Thumbnail URL must use https://";if(a.imageUrl&&!/^https:\/\//i.test(a.imageUrl))return"Banner URL must use https://";const cr=await fetch("https://discord.com/api/v10/channels/"+a.channelId,{headers:botHeaders(env)});if(!cr.ok)return"Could not access selected channel";const ch=await cr.json();if(String(ch.guild_id||"")!==String(guildId)||![0,5].includes(ch.type))return"Select a text or announcement channel from this server";if(a.crosspost&&ch.type!==5)return"Follower publishing requires a Discord Announcement channel";return null}
async function saveAnnouncement(req,env,guildId,id=""){let body;try{body=await req.json()}catch{return json({error:"Invalid JSON"},400)}const a=parseAnnouncementBody(body),error=await validateAnnouncement(env,guildId,a);if(error)return json({error:error},400);await ensureAnnouncementTables(env);const now=new Date().toISOString();if(id){const old=await env.BALTICM_DB.prepare("SELECT id,channel_id AS channelId FROM announcements WHERE id=? AND guild_id=?").bind(id,guildId).first();if(!old)return json({error:"Announcement not found"},404);const channelChanged=String(old.channelId||"")!==String(a.channelId||"");await env.BALTICM_DB.prepare("UPDATE announcements SET channel_id=?,title=?,content=?,thumbnail_url=?,image_url=?,color=?,footer=?,buttons_json=?,crossposted=?,message_id=CASE WHEN ? THEN '' ELSE message_id END,status=CASE WHEN ? THEN 'draft' ELSE status END,updated_at=? WHERE id=? AND guild_id=?").bind(a.channelId,a.title,a.content,a.thumbnailUrl,a.imageUrl,a.color,a.footer,JSON.stringify(a.buttons),a.crosspost?1:0,channelChanged?1:0,channelChanged?1:0,now,id,guildId).run();return json({ok:true,id:id})}id=crypto.randomUUID();await env.BALTICM_DB.prepare("INSERT INTO announcements (id,guild_id,channel_id,title,content,thumbnail_url,image_url,color,footer,buttons_json,message_id,crossposted,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,'',?,'draft',?,?)").bind(id,guildId,a.channelId,a.title,a.content,a.thumbnailUrl,a.imageUrl,a.color,a.footer,JSON.stringify(a.buttons),a.crosspost?1:0,now,now).run();return json({ok:true,id:id},201)}
async function publishAnnouncement(env,guildId,id){await ensureAnnouncementTables(env);const row=await env.BALTICM_DB.prepare("SELECT id,channel_id AS channelId,title,content,thumbnail_url AS thumbnailUrl,image_url AS imageUrl,color,footer,buttons_json AS buttonsJson,message_id AS messageId,crossposted FROM announcements WHERE id=? AND guild_id=?").bind(id,guildId).first();if(!row)return json({error:"Announcement not found"},404);const a=announcementRow(row),embed={color:a.color||7624695};if(a.title)embed.title=a.title;if(a.content)embed.description=a.content;if(a.thumbnailUrl)embed.thumbnail={url:a.thumbnailUrl};if(a.imageUrl)embed.image={url:a.imageUrl};if(a.footer)embed.footer={text:a.footer};const components=a.buttons.length?[{type:1,components:a.buttons.map(b=>{const c={type:2,style:5,label:b.label,url:b.url};if(b.emoji)c.emoji={name:b.emoji};return c})}]:[],payload={embeds:[embed],components:components,allowed_mentions:{parse:[]}};let r;if(a.messageId){r=await fetch("https://discord.com/api/v10/channels/"+a.channelId+"/messages/"+a.messageId,{method:"PATCH",headers:botHeaders(env,true),body:JSON.stringify(payload)});if(r.status===404)r=await fetch("https://discord.com/api/v10/channels/"+a.channelId+"/messages",{method:"POST",headers:botHeaders(env,true),body:JSON.stringify(payload)})}else r=await fetch("https://discord.com/api/v10/channels/"+a.channelId+"/messages",{method:"POST",headers:botHeaders(env,true),body:JSON.stringify(payload)});const data=await r.json().catch(()=>({}));if(!r.ok)return json({error:data.message||"Could not publish announcement",status:r.status},502);let crossposted=false,crosspostError="";if(a.crossposted){const xr=await fetch("https://discord.com/api/v10/channels/"+a.channelId+"/messages/"+data.id+"/crosspost",{method:"POST",headers:botHeaders(env)});crossposted=xr.ok;if(!xr.ok){const xd=await xr.json().catch(()=>({}));crosspostError=xd.message||"Discord crosspost failed"}}const now=new Date().toISOString();await env.BALTICM_DB.prepare("UPDATE announcements SET message_id=?,status='published',published_at=?,updated_at=? WHERE id=? AND guild_id=?").bind(data.id,now,now,id,guildId).run();return json({ok:true,messageId:data.id,crossposted:crossposted,crosspostError:crosspostError})}
async function deleteAnnouncement(env,guildId,id){await ensureAnnouncementTables(env);const a=await env.BALTICM_DB.prepare("SELECT channel_id AS channelId,message_id AS messageId FROM announcements WHERE id=? AND guild_id=?").bind(id,guildId).first();if(!a)return json({error:"Announcement not found"},404);if(a.messageId)await fetch("https://discord.com/api/v10/channels/"+a.channelId+"/messages/"+a.messageId,{method:"DELETE",headers:botHeaders(env)}).catch(()=>null);await env.BALTICM_DB.prepare("DELETE FROM announcements WHERE id=? AND guild_id=?").bind(id,guildId).run();return json({ok:true})}
...
async function saveAccessSettings(req,env,guildId){
 try{
  const body=await req.json(),input=Array.isArray(body.roles)?body.roles:[];
  const roles=input.slice(0,50).map(r=>({roleId:String(r.roleId||""),enabled:r.enabled!==false,permissions:ACCESS_KEYS.filter(k=>Array.isArray(r.permissions)&&r.permissions.includes(k))})).filter(r=>/^\d{16,22}$/.test(r.roleId));
  const config={roles};await env.BALTICM_DB.prepare("CREATE TABLE IF NOT EXISTS bot_config (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TEXT NOT NULL)").run();
...
if(p==="/api/admin/support/reply"&&req.method==="POST")return adminSupportReply(req,env,user);
if(p==="/api/admin/support/status"&&req.method==="POST")return adminSupportStatus(req,env,user);
if(p==="/api/support-chat"){if(req.method==="GET")return supportChatState(req,env,user);if(req.method==="POST")return supportChatSend(req,env,user);return json({error:"Method not allowed"},405);}
