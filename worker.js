...
async function publishReactionRolePanel(env,guildId,id){
 ...
 const cleanDescription=p.thumbnailUrl?description.replaceAll(p.thumbnailUrl,"").replace(/^\s+|\s+$/g,"").replace(/\n{3,}/g,"\n\n"):description;\n const embed={title:p.title,description:cleanDescription,color:0x7457ff};if(p.thumbnailUrl)embed.image={url:p.thumbnailUrl};const payload={embeds:[embed],allowed_mentions:{parse:[]}};
